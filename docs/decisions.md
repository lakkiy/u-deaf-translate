...
### 三后端：Chrome / DeepSeek / 自定义，options 页 tab 互斥选择
`config.active: 'chrome' | 'deepseek' | 'custom'` 决定 `translate()` 走哪条路径。DeepSeek 的 endpoint 和生成参数在 `src/deepseek.ts` 写死，用户只需要填 API Key 和从下拉选 model（`deepseek-v4-flash` / `deepseek-v4-pro`，默认 flash）——避免新手在自定义 tab 一个字段一个字段配置又填错。`DEEPSEEK_MODELS` 数组列出可选值，新增/调整模型只改这个数组（select 自动 populate）。自定义 tab 保留完整 5 字段。三个后端的配置在 storage 各自独占字段（`deepseekApiKey` / `deepseekModel` / `custom: {...}`），切 tab 不丢任何 tab 的输入。所有字段都在 DOM 里渲染，切 tab 只调 `.active` class，state 天然 persist。
...
### Prompt 模板用 `{target_lang}` / `{source_text}` 占位符
`config.promptTemplate` 默认 Hy-MT2 推荐格式：`将以下文本翻译为 {target_lang}，注意只需要输出翻译后的结果，不要额外解释：\n\n{source_text}`。渲染用 `replaceAll`，不做转义——LLM 自身能处理特殊字符，用户填的模板我们尊重。Options 页面保存时如果模板缺占位符给软警告（仍允许保存）。
...
### 互译：选中文本属于 PRIMARY 语族 → 译为 SECONDARY，否则 → 译为 PRIMARY
两个常量：`PRIMARY_TARGET_LANGUAGE = 'zh-Hans'`（默认目标）和 `SECONDARY_TARGET_LANGUAGE = 'en'`（回译语言）。`pickTargetLanguage(source)` 用 `sameLanguageFamily()` 判断——比较 BCP-47 主子标签（`zh-Hans` / `zh-CN` / `zh-TW` 都算同一族），属于 PRIMARY 族就译为 SECONDARY，否则译为 PRIMARY。这样选英文/法文/任意外语都翻成中文，选中文翻成英文。将来加配置 UI 只改这两个常量即可。
